...
int main()
{
    ...
    printf("  Total global memory (B)    : %zu\n", dev_property.totalGlobalMem);
		...
		printf("  Memory clock frequency (kHz)       : %d\n", dev_property.memoryClockRate);
		printf("  Memory bus width (bits)            : %d\n", dev_property.memoryBusWidth);
		printf("  Theoretical memory bandwidth (GB/s): %f\n", 
					(dev_property.memoryClockRate * 1e3) * (dev_property.memoryBusWidth * 2) * (1/8.0) * (1/1.0e9) );
